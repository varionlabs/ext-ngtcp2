...
ZEND_BEGIN_ARG_INFO_EX(arginfo_datagram_construct, 0, 0, 2)
  ...
  ZEND_ARG_OBJ_INFO(0, peerAddress, Varion\\Ngtcp2\\Address, 0)
  ZEND_ARG_OBJ_INFO_WITH_DEFAULT_VALUE(0, localAddress, Varion\\Ngtcp2\\Address, 1, "null")
  ZEND_ARG_TYPE_INFO_WITH_DEFAULT_VALUE(0, ecn, IS_LONG, 1, "null")
  ZEND_ARG_TYPE_INFO_WITH_DEFAULT_VALUE(0, receivedAt, IS_LONG, 1, "null")
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_datagram_get_payload, 0, 0, IS_STRING, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_datagram_get_peer, 0, 0, Varion\\Ngtcp2\\Address, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_OBJ_INFO_EX(arginfo_datagram_get_local, 0, 0, Varion\\Ngtcp2\\Address, 1)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_datagram_get_ecn, 0, 0, IS_LONG, 1)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_WITH_RETURN_TYPE_INFO_EX(arginfo_datagram_get_received_at, 0, 0, IS_LONG, 1)
ZEND_END_ARG_INFO()
...
PHP_METHOD(Ngtcp2_Datagram, __construct) {
  ...
  zend_long ecn = 0;
  zend_long received_at = 0;
  zend_bool ecn_is_null = 1;
  zend_bool received_at_is_null = 1;

  ZEND_PARSE_PARAMETERS_START(2, 5)
    Z_PARAM_STR(payload)
    Z_PARAM_OBJECT_OF_CLASS(peer_address, php_quic_address_ce)
    Z_PARAM_OPTIONAL
    Z_PARAM_OBJECT_OF_CLASS_OR_NULL(local_address, php_quic_address_ce)
    Z_PARAM_LONG_OR_NULL(ecn, ecn_is_null)
    Z_PARAM_LONG_OR_NULL(received_at, received_at_is_null)
  ...
  ZVAL_COPY(&datagram->peer_address, peer_address);
  ...
  datagram->has_ecn = !ecn_is_null;
  datagram->ecn = ecn;
  datagram->has_received_at = !received_at_is_null;
  datagram->received_at = received_at;
}
...
PHP_METHOD(Ngtcp2_Datagram, getPeerAddress) {
  php_quic_datagram *datagram = Z_QUIC_DATAGRAM_P(ZEND_THIS);
  RETURN_COPY(&datagram->peer_address);
}
...
PHP_METHOD(Ngtcp2_Datagram, getEcn) {
  php_quic_datagram *datagram = Z_QUIC_DATAGRAM_P(ZEND_THIS);
  if (!datagram->has_ecn) {
    RETURN_NULL();
  }

  RETURN_LONG(datagram->ecn);
}

PHP_METHOD(Ngtcp2_Datagram, getReceivedAt) {
  php_quic_datagram *datagram = Z_QUIC_DATAGRAM_P(ZEND_THIS);
  if (!datagram->has_received_at) {
    RETURN_NULL();
  }

  RETURN_LONG(datagram->received_at);
}
...
static const zend_function_entry php_quic_datagram_methods[] = {
  ...
  PHP_ME(Ngtcp2_Datagram, getPeerAddress, arginfo_datagram_get_peer, ZEND_ACC_PUBLIC)
  PHP_ME(Ngtcp2_Datagram, getLocalAddress, arginfo_datagram_get_local, ZEND_ACC_PUBLIC)
  PHP_ME(Ngtcp2_Datagram, getEcn, arginfo_datagram_get_ecn, ZEND_ACC_PUBLIC)
  PHP_ME(Ngtcp2_Datagram, getReceivedAt, arginfo_datagram_get_received_at, ZEND_ACC_PUBLIC)
  PHP_FE_END
};
...
static zend_object *php_quic_datagram_create_object(zend_class_entry *ce) {
  ...
  ZVAL_UNDEF(&datagram->peer_address);
  ZVAL_UNDEF(&datagram->local_address);
  datagram->ecn = 0;
  datagram->received_at = 0;
  datagram->has_ecn = 0;
  datagram->has_received_at = 0;
